...
if(this.props.match.params.curriculumid) {
    initialDays = this.props.curricula.filter( curriculum => curriculum.id === +this.props.match.params.curriculumid)[0]
...
if( curriculumErrors.length > 0 ) {
    ...
    if( params ) {
        axios.put(`/api/curriculum/${params}`, curriculumBody)
        .then( this.props.history.goBack() )
    } else {
        axios.post('/api/curriculum/', curriculumBody)
