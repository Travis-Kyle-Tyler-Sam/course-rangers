...
class CourseBuilderTool extends Component {
  ...
  handleDaySelected(day) {
    ...
    let assignedDate = moment(this.props.startDate)
      .subtract(1, "d")
      .format("YYYY-MM-DD");
  ...
  submitCourse = async () => {
    ...
    if (errors.length > 0) {
      ...
      let selectedCurriculum = this.props.curricula.filter(
        curriculum => curriculum.id === this.props.template
      );
      selectedCurriculum = selectedCurriculum[0];
      let filteredPopDays = this.state.popDays.filter(
        popDay => popDay.type === "attend"
      );
      ...
      if (this.props.routeParams) {
        await axios.put(
          "/api/course/" + this.props.routeParams,
          selectedCurriculum
        );
      } else {
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="cbt-btn-container">
          <Button
            primary
            size="large"
...
export default connect(mapStateToProps, { updateCourseStudents })(
  CourseBuilderTool
);
